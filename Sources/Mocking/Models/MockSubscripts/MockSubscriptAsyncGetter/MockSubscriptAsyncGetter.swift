//
//  MockSubscriptAsyncGetter.swift
//
...
/// records for an async subscript getter.
public final class MockSubscriptAsyncGetter<Arguments, Value> {
    ...
    /// invocation records for an async subscript getter.
    ///
    ...
    func get(_ arguments: Arguments) async -> Value {
        guard let value = await self.implementation(arguments) else {
...
extension MockSubscriptAsyncGetter: Sendable where Arguments: Sendable, Value: Sendable {}
