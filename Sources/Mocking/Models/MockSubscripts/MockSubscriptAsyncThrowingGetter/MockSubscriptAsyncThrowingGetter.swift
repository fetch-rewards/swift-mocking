//
//  MockSubscriptAsyncThrowingGetter.swift
//
...
/// records for an async, throwing subscript getter.
public final class MockSubscriptAsyncThrowingGetter<Arguments, Value> {
    ...
    /// invocation records for an async, throwing subscript getter.
    ///
    ...
    func get(_ arguments: Arguments) async throws -> Value {
        let value = await Result {
            guard let value = try await self.implementation(arguments) else {
...
extension MockSubscriptAsyncThrowingGetter: Sendable where Arguments: Sendable, Value: Sendable {}
