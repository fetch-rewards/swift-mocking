...
struct MockReturningParameterizedAsyncMethodTests {
    ...
    typealias Closure = @Sendable (String, Bool) async -> ReturnValue
    ...
    func callCount() async throws {
        ...
        try await TestBarrier.executeConcurrently {
            let returnValue = await invoke("a", true)
        ...
        try await TestBarrier.executeConcurrently {
            let returnValue = await invoke("b", false)
            recordOutput(returnValue)
        }
        #expect(sut.callCount == TestBarrier.defaultTaskCount * 2)
    ...
    func returnedValues() async throws {
        ...
        try await TestBarrier.executeConcurrently {
            let returnValue = await invoke1("a", true)
            recordOutput(returnValue)
        }
        ...
        try await TestBarrier.executeConcurrently {
            let returnValue = await invoke2("b", false)
            recordOutput(returnValue)
        }
