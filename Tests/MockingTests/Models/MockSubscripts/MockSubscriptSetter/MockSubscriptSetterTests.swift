//
//  MockSubscriptSetterTests.swift
//
...
struct MockSubscriptSetterTests {
    ...
    func invocations() async throws {
        ...
        #expect(
            sut.invocations.suffix(TestBarrier.defaultTaskCount).allSatisfy { invocation in
                invocation.0 == "b" && invocation.1 == 10
            }
        )
    ...
    func lastInvocation() async throws {
        ...
        #expect(sut.lastInvocation?.0 == "a")
        #expect(sut.lastInvocation?.1 == 5)

        try await TestBarrier.executeConcurrently {
            set("b", 10)
        }
        #expect(sut.lastInvocation?.0 == "b")
        #expect(sut.lastInvocation?.1 == 10)
